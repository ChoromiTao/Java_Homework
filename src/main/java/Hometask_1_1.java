...
//        System.out.println(Arrays.toString(array_min_value));
    ...
    public static int[] arr_max_value(int i, int bit_length){
        int arr_length = 0;
        for (int j = i; j < (Short.MAX_VALUE) & (j%bit_length == 0); j++) {
            arr_length ++;
        }
        int[] arr_max_value = new int[arr_length];
        for (int j = i, k = 0; j < (Short.MAX_VALUE) & j%bit_length == 0; j++) {
            arr_max_value[k] = j;
            k++;
        }
    ...
    public static int[] arr_min_value(int i, int bit_length){
        int[] arr_min_value = new int[Math.abs(Short.MIN_VALUE)];
        for (int j = Short.MIN_VALUE, k = 0; j < i & bit_length % j != 0; j++) {
            arr_min_value[k] = j;
            k++;
        }
